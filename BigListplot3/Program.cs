    ...
    static async Task Main(string[] args)
    {
        ...
        using (StreamWriter sw = new StreamWriter("delays_with_timestamps.csv"))
        {
            ...
            for (int i = 0; i < list1.Count; i++)
            {
                ...
                genericJsonObject = JsonDocument.Parse(valuesToSend);
                ...
                { 
                    ...
                    operational_data_patch.AppendAdd<string>("", valuesToSend[j].GetDouble().ToString());
                     patchList.Add(operational_data_patch);

                    string twinId = updateTwinIds[j % updateTwinIds.Count];
                    string value = valuesToSend[j];
                    for(int k=0; k<patchList.Count; k++){
                        m_azureClient.UpdateComponent(twinIds[k], "value", patchList[k]); 
