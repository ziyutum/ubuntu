...
int main(int argc, char** argv) {
    ...
    csv_file << "timestamp, q1, q2, q3, q4, q5, q6, q7, Extra1, Extra2" << std::endl;
...
//         // this is the initial_pos1 
         ...
         auto cartesian_initial_motion_0 = [&initial_pos0, &time0, &finish_time0, &client_socket,&csv_file](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
        {
            ...
            //double dz = -0.1*(cos(M_PI_4*(1-cos(M_PI/5*time1))));
             ...
             double dx = 0 * (cos(M_PI_4*(1-cos(M_PI/3*time0)))-1);
            ...
            if(time0 >= finish_time0)
            {
             ...
             auto microseconds = now_us.time_since_epoch().count() % 1000000;

            std::stringstream timestamp_ss;
            timestamp_ss << std::put_time(std::localtime(&now_c), "%Y-%m-%d %H:%M:%S") << "." << std::setw(6) << std::setfill('0') << microseconds;
...
//             // previously it was std::string test
                ...
                test += "]\r\n";
               //test += "Timestamp: " + timestamp_ss.str();
                test += "}\r\n";
                std::cout << "send message to client:"<< test.c_str()<<test.length() << std::endl;
                //std::cout << test.length()<< std::endl; 
                //std::cout << test<< std::endl;  
                ...
                csv_file << timestamp_ss.str();
                for (double q : rstate.q) {
                    csv_file << ", " << q;
                }
                csv_file << ", 0.01, 0.01" << std::endl;
