    ...
    auto cartesian_motion = [&time, &total_time, &initial_pose, &targetCoordinates, &client_socket, &GW, &update_count](const franka::RobotState& robot_state,
        ...
        for (int i = 0; i < 9; i++) {
            if (i < 7) {
                ...
                test += std::to_string(GW); // Feng Xu 08.07.2024
                if (test.length() < 100) {
...
int main(int argc, char** argv) {
    ...
    try
    {
        ...
        std::array<double, 7> safepos = {{1.79302, -0.422361, 0.406882, -1.87294, 0.168881, 1.41577, 1.11039}};
        ...
        std::vector<double> targetCoordinates1 = {0.0, 0.0, 0.33};
        gripper.move(0.05, 0.01);
        double movementTime1 = 5.0;
        ...
        gripper.grasp(0.03, 0.02, 70);

        // 3. Move around
        std::cout << "3. Move around" << client_socket << std::endl;
        std::vector<double> targetCoordinates2 = {-0.657, -0.0215, -0.26};
        double movementTime2 = 5.0;
        ...
        // 4: place object
        std::vector<double> targetCoordinates4 = {0.0, 0.0, 0.39};
        double movementTime4 = 5.0;
